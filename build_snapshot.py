...
def _banked_backfill_findings():
    ...
    specs = [
        dict(
            ...
            headline="RED-TEAM WOUNDED: ~73% of the +0.039 is a 'is-this-village-monitored' flag; Alexandrium-abundance-only cold-start is NULL (dAP +0.062, CI[-0.007,0.102], perm p=0.62)",
            ...
            how="Leave-site-out cold start. Raw-data red-team decomposed the signal: a pure monitoring-presence indicator recovers +0.028 of the +0.039; abundance-only co-located slice is null; within-site key-shuffle leaves a phantom +0.024.",
            metrics={"n_site_days": 2549, "independent_exceedance_runs": 265, "n_sites": 29, "base_rate": 0.322,
                     "headline_marginal_ap": 0.0386, "hasobs_only_marginal_ap": 0.028,
                     "abundance_only_cold_ci95": [-0.007, 0.102], "abundance_only_perm_p": 0.62,
                     "forward_split_marginal_ap": -0.0008, "redteam_2026_07_14": "WOUNDED->near-REFUTED"},
            note="Disjoint non-circular sources are real, and the forward split is honestly NULL. But the detection headline is a monitoring-COVERAGE artifact: the abundance-only cold-start test is a null (CI crosses 0, perm p=0.62) and the '820 events' were really 265 independent runs. Do NOT treat as a deployable detector.",
        ),
        dict(
        ...
        dict(
            ...
            headline="SST adds ~+0.05-0.07 marginal AP at 2wk lead; year-clustered CI[+0.014,+0.120] over 7 forward years (replicates 2023-25)",
            plain="Long coastal water temperature adds a real but modest forward improvement over season+persistence for state-week Vibrio infection spikes two weeks ahead. It holds because it independently replicates across seven forward years, not because of any single year.",
            how="Rolling-origin forward by year, 8 states, lead 2wk. RED-TEAM: the published state-clustered CI overstates precision -- re-clustered on the scarce YEAR unit, the 2018-2021 window crosses 0 ([-0.052,+0.201]) and rests on 2019; survival comes from post-COVID replication (2023/24/25).",
            metrics={"n_states": 8, "marginal_ap_7yr": 0.067, "ci95_year_clustered_7yr": [0.014, 0.120],
                     "ci95_year_clustered_window": [-0.052, 0.201], "year_2018_ap": -0.037, "year_2019_ap": 0.147,
                     "forward_years_positive": "6 of 7", "redteam_2026_07_14": "SURVIVES (precision reframed)"},
            note="Real, modest effect that replicates 2023-25. The originally-published tight CI[0.021,0.115] was state-clustered; the honest year-clustered CI is [+0.014,+0.120] over 7 years. Uses finalized annual NNDSS, so live deployment would be noisier.",
        ),
        dict(
            ...
            headline="forward skill vs TREND +0.69 (bootstrap CI[+0.48,+0.85]); cross-buoy range E01 ~0.70 / M01 ~0.30-0.54",
            plain="April-May bottom-water temperature, known by June 1, forecasts when the summer Maine lobster landings peak -- useful for ex-vessel price and market planning. A directional, pilot-grade signal, not a calibrated forecast.",
            how="Held-out unit = YEAR, rolling-origin. RED-TEAM: survives year-jackknife (11/13 yrs), trend-preserving and AR1 shuffle nulls (p=0.0003), and a volume-confound test; but 0.705 is E01-buoy-specific -- the corroborating M01 buoy gives ~0.30-0.54.",
            metrics={"skill_vs_trend": 0.69, "skill_ci95": [0.48, 0.85], "skill_vs_clim_e01": 0.705,
                     "skill_m01_buoy": "0.30-0.54", "shuffle_ar1_p": 0.0003, "n_years": 21, "n_test_years": 13,
                     "redteam_2026_07_14": "SURVIVES (range + CI added)"},
            note="Genuine forward, non-circular, trend-beating signal -- but pilot-grade: quote the cross-buoy range (E01 ~0.70 / M01 ~0.30-0.54) and CI[+0.48,+0.85], not a lone 0.705. Timing is from landings (molt vs fishing-effort unseparable at monthly grain).",
        ),
        dict(
        ...
        dict(
            ...
            headline="CA vs FL P(fatal|bite) 3.1x (upper est.; robust floor ~2.4x); modern-only gap WIDENS to 9.3x (p=1e-7); CA=white-shark regime, FL=small-shark",
            ...
            how="Global Shark Attack File, severity conditional on an unprovoked bite. RED-TEAM: gap gets STRONGER under modern-era-only control (9.3x); the 'CA over-records severe bites' attack refuted (CA has MORE trivial bites); but P(fatal|recorded bite) is not recording-invariant (FL fell 0.13->0.007 as its record grew).",
            metrics={"n_unprovoked": 4686, "ca_p_fatal": 0.0896, "ca_p_fatal_ci": [0.0581, 0.1357],
                     "fl_p_fatal": 0.0291, "fl_p_fatal_ci": [0.0202, 0.0417], "worst_case_floor_ratio": 2.4,
                     "modern_only_ratio": 9.3, "modern_only_p": 1e-7, "redteam_2026_07_14": "SURVIVES (framing softened)"},
            note="Direction (CA rare-lethal-white vs FL common-minor-small) is the bankable claim (non-overlapping CIs, p=4.6e-5). 'Exposure-invariant' overstates: denominator-free is true, recording-bias-free is not -- so 3.1x is an upper estimate, floor ~2.4x. Pinniped mechanism UNCONFIRMED.",
        ),
        dict(
        ...
        dict(
            ...
            headline="RED-TEAM: the sole 'fault' flag is Monterey UPWELLING misread as drift -- the buoy matches the M1 mooring to 0.01C (r=0.968) -- and it no longer fires on current data. Precision on the only adjudicable case = 0/1",
            plain="A tool meant to catch broken coastal sensors by comparing each to its neighbors. Its one demo 'catch' was actually a correctly-working buoy reading real upwelling, and the alert disappears on newer data. The useful part is the map of where cross-checking is even possible.",
            how="RED-TEAM pulled the flagged buoy vs neighbors from the raw 1.4B-row layer: it agrees with the one true cross-source neighbor (M1 mooring) year-round; the 'divergence' was against 6 same-source warm inner-bay buoys during upwelling season. Re-running the shipped detector on current data yields 0 flags.",
            metrics={"demo_n_flagged": 1, "precision_on_adjudicable_case": 0.0, "flag_vs_m1_mooring_corr": 0.968,
                     "flag_vs_m1_residual_C": 0.01, "flags_on_current_data": 0, "labeled_true_faults": 0,
                     "sst_cross_source_pct_10km": 20.6, "redteam_2026_07_14": "REFUTED as detector"},
            note="The buddy-check does NOT work as a fault detector as built (neighbors weren't comparable; flags the most oceanographically-correct sensor in an upwelling system). What SURVIVES: the redundancy census (SST 20.6% / salinity 12% / DO 0% cross-source) and the univariate QARTOD self-checks. Needs comparable-neighbor selection + labeled faults before any claim.",
        ),
        dict(
            ...
            metrics={"n_pairs": 45, "pooled_auc": 0.609, "auc_ci95": [0.493, 0.725], "sign_p_one_sided": 0.033,
                     "sign_p_two_sided": 0.066, "non_karenia_negatives": 0, "available_pairs_unused": 1460,
                     "redteam_2026_07_14": "REFUTED (no CI-separated skill)"},
            note="No CI-separated evidence of skill -- the number should not be read as a 'detector'. What is real is the METHODS lesson: same-station pairing wasn't enough; a season-match caught a 0.81->0.61 seasonal inflation. Consistent with the known dense-biomass/RBD effect but this test does not establish Karenia specificity.",
        ),
        dict(
            fid="banked_josh_fib_trend", kind="caveat",
            evidence="analysis/josh_bacteria_trends_2026-07-10/analysis_scripts/results.json",
            title="CA beach-bacteria trend (MBARI/Josh): improving, not flat",
            headline="RED-TEAM: 'no trend' is overstated -- corrected regulatory data show well-powered station-level DECLINES in enterococcus (ex-SD p=3e-6) and fecal coliform (p=3.4e-9). There is no INCREASE; weather dominates day-to-day",
            plain="California regulatory beach bacteria is not flat -- it is slowly improving. Two indicators show real declines once you use the powered station-level test and remove a San Diego reporting glitch. The apparent rise in advisories is a counting artifact, and rainfall dominates day-to-day spikes.",
            how="Per-station Kendall-tau on the balanced panel (powered) vs the underpowered 26-point annual Mann-Kendall used originally. RED-TEAM: the earlier enterococcus 'null' was a San Diego ddPCR contamination in the silver source; ex-SD the same test shows a decline (p=3e-6).",
            metrics={"fecal_coliform_censored_p": 3.4e-9, "entero_decline_exSD_p": 3e-6,
                     "annual_mk_entero_p": 0.201, "annual_mk_note": "underpowered 26-pt test",
                     "wet_dry_ratio": "1.7-2.9x", "advisory_unknown_cause_pct": 63.2,
                     "surfrider_volunteer": "disagrees (shows increase)", "redteam_2026_07_14": "NULL OVERSTATED -> declines"},
            note="What survives: no statewide INCREASE, weather dominates, advisory-count rise is a reporting artifact. What was wrong: 'no robust trend' buried two well-powered declines; the flat annual test is underpowered and the enterococcus 'null' was a San Diego ddPCR artifact. Volunteer Surfrider data disagrees (shows an increase). Re-run 07_censored_trend on the method-aware source before citing.",
        ),
        dict(
